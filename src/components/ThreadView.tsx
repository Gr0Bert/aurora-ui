...
import type { RunStatus, TaskSnapshot, ThreadGraph } from "../types";
import { DebugDrawer } from "./DebugDrawer";

type PendingApproval = { task: TaskSnapshot; rootRunId: string; childLabel?: string };
...
// A parent suspended on a delegated child's HITL approval is `yielded` while a
// descendant run is parked (its own `waiting_for_task`, or `yielded` because a
// deeper descendant is parked). Surface that as "waiting on child".
function waitingOnChild(runs: ThreadGraph["runs"], runId: string): boolean {
  return runs.some(
    (r) =>
      r.parent_run_id === runId &&
      (r.status === "waiting_for_task" || r.status === "yielded"),
  );
}
...
export function ThreadView({
  ...
  // Collect pending approvals across the run tree (root + delegated children),
  // mapping each to its root run so the button can sit next to that exchange.
  const loadPending = useCallback(async (g: ThreadGraph) => {
    const runs = g.runs ?? [];
    const waiting = runs.filter((r) => r.status === "waiting_for_task");
    const lists = await Promise.all(
      waiting.map((r) =>
        api
          ...
          .catch(() => [] as { task: TaskSnapshot; run: ThreadGraph["runs"][number] }[]),
      ),
    );
    ...
    const rootOf = (id: string): string => {
      let cur = byId.get(id);
      let guard = 0;
      while (cur && cur.parent_run_id && guard++ < 64) cur = byId.get(cur.parent_run_id);
      return cur ? cur.run_id : id;
    };
    setPending(
      lists.flat().map(({ task, run }) => ({
        task,
        rootRunId: rootOf(run.run_id),
        childLabel: run.parent_run_id
          ? run.message?.slice(0, 48) || run.run_id.slice(0, 8)
          : undefined,
      })),
    );
  }, []);
  ...
  }, [threadID, handleError, loadPending]);

  const resolve = useCallback(
    async (task: TaskSnapshot, decision: "approved" | "denied") => {
      setBusy(true);
      try {
        await api.resolveTask(task.id, task.webhook_token, { decision, actor: "ui" });
        await reload();
      } catch (e) {
        handleError(e);
      } finally {
            ...
            {rootRuns.map((run) => (
              <div key={run.run_id} className={`exchange${run.run_id === activeDrawerRunID ? " active" : ""}`}>
                ...
                <div className="run-meta">
                  ...
                  {run.status === "yielded" && waitingOnChild(graph?.runs ?? [], run.run_id) ? (
                    <StatusBadge status="waiting_for_task" label="waiting on child" />
                  ) : (
                ...
                {pending
                  .filter((p) => p.rootRunId === run.run_id)
                  .map((p) => (
                    <div key={p.task.id} className="tasks inline-approval">
                      <div className="task">
                        {p.childLabel && (
                          <div className="task-call">↳ child · {p.childLabel}</div>
                        )}
                        <div className="task-summary">
                          {p.task.summary || p.task.call?.name || "Approval required"}
                        </div>
                        {p.task.call?.name && (
                          <code className="task-call">{p.task.call.name}</code>
                        )}
                        <div className="task-actions">
                          <button
                            className="approve"
                            disabled={busy}
                            onClick={() => void resolve(p.task, "approved")}
                          >
                            Approve
                          </button>
                          <button
                            className="deny"
                            disabled={busy}
                            onClick={() => void resolve(p.task, "denied")}
                          >
                            Deny
                          </button>
                        </div>
